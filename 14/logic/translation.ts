...
const darkenColor = (hex: string, factor: number = 0.85): string => {
  ...
  return rgbaToHex(rgb.r * factor, rgb.g * factor, rgb.b * factor, 255);
};
...
/**
 * 5. Preserve Thin Features (Anti-Erosion)
 * Protects 1px wide limbs/tails/items from being cleaned up as "noise".
 */
const preserveThinFeatures = (grid: string[][]) => {
  const h = grid.length;
  const w = grid[0].length;
  const out = grid.map((r) => [...r]); // Copy for checking context

  // Note: We modify 'out' but read from 'grid' to ensure atomic decisions
  // However, for preservation, we actually just want to mark them "safe".
  // The Cleanup function needs to run AFTER this.
  // Since cleanup looks at neighbors, we don't need to change data here,
  // we just need to ensure cleanup logic respects thin lines.
  // BUT, to keep it simple, we will use this phase to perhaps "thicken" extremely thin
  // crucial lines if needed, OR we just trust the improved cleanup logic below.

  // Actually, per prompt instruction: "Before cleanupIslands, detect 1px-wide vertical or horizontal runs".
  // The provided snippet logic was to skip cleanup for these.
  // I will integrate this check INTO cleanupIslands for efficiency and correctness.
  return out;
};
